...
Supabase環境変数が設定されている場合だけ、クラウドバックアップ機能を有効化します。設定画面の「バックアップ」からローカルバックアップとクラウドバックアップを同じ画面で管理します。未ログイン時は同画面内に新規登録/ログインフォームを表示し、ログイン後はクラウドバックアップ一覧を表示します。
...
- Supabase未設定時:
  ...
  - バックアップ画面には「Supabaseの設定がないため無効」と表示します。
- 認証:
  ...
  - バックアップ画面からクラウドアカウントを削除できます。削除すると `auth.users` のユーザーを削除し、`on delete cascade` により `profiles`、`devices`、`cloud_backups` の該当データも削除します。端末内のローカルデータは削除しません。
- バックアップ:
  - ユーザーが手動で実行します。
...
- 初期表示では「成長グラフ」「総ボリューム」「自己ベスト」「実施回数」のページリンクを表示し、タップした分析内容へ遷移する。各分析内容からは分析メニューへ戻れる。
...
- クラウドバックアップでは、未ログイン時に新規登録/ログインフォームを表示し、ログイン後にバックアップ作成、一覧更新、復元、個別削除を実行できる。
- ログイン後のバックアップ画面下部には「アカウント管理」リンクを表示し、パスワード変更とアカウント削除は独立したアカウント管理画面で実行できる。

### 6.17 アカウント管理（`AccountManagementScreen`）

- バックアップ画面の「アカウント管理」から遷移する。トップバーの戻るでバックアップ画面へ戻る。
- Supabase未設定時はクラウドアカウント管理が無効である旨を表示する。
- 未ログイン時はログインが必要である旨を表示し、バックアップ画面へ戻るボタンを表示する。
- ログイン中はログイン中のメールアドレス、新しいパスワード入力、確認入力、パスワード変更ボタンを表示する。
