...
| JSON バックアップ/復元 | 記録を書き出し、記録削除後に JSON を読み込んで復元する | local-first の退避・復元導線が実ファイル操作込みで動くことを保証する |
| 終了/再開 | セット入力後にトレーニングを終了し、読み取り専用表示を確認してから再開する | 終了状態と編集可能状態の切り替えが壊れないことを保証する |
| 履歴/分析反映 | 入力済み記録を履歴、成長グラフ、自己ベスト、実施回数で確認する | 保存された記録が別画面の集計表示へ反映されることを保証する |
| 種目マスタ | 設定から種目を追加、改名、削除する | マスタ編集と一覧反映が実ブラウザ操作でつながることを保証する |
| トレーニングメニュー | メニューを作成し、複数種目を選んでホームから開始する | プリセット作成からワークアウト一括投入までの導線を保証する |

### 重点的に守っている挙動

- E2E はテストごとに `localStorage` を消し、初期状態から開始する。
- 入力後は `localStorage` に保存されたことを待ってからリロードし、保存デバウンスによる不安定さを避ける。
- JSON 復元では成功トーストの順序に依存せず、復元後の保存データとホーム表示を確認する。
- 実通信を伴う Firebase 操作は行わず、クラウドバックアップは導線表示までを確認する。
