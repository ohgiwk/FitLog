...
- [x] バックアップ一覧画面からクラウドアカウントを削除できるようにする。
...
アカウント削除は `delete_current_user()` RPC から `auth.users` のログイン中ユーザーを削除し、`on delete cascade` でクラウド側の関連データを削除する。端末内の `localStorage` は削除しない。
