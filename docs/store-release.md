...
- アカウント作成を提供する場合、アプリ内またはWeb上でアカウント削除を開始できる導線が必要。Firebase Auth ユーザーと Firestore バックアップの削除方針を実装・説明する。
...
- 暗号化の利用有無を回答する。HTTPS / Firebase 通信を使うため、Export Compliance の確認が必要。
...
3. アカウント削除導線を設計・実装する。
   ...
   - Firebase Auth ユーザー、Firestore の `users/{uid}` 配下データの削除範囲を決める
