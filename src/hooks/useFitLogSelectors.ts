...
import { buildPartRecentLabels, buildSplitPartOptions, plannedPartsForDate } from "../selectors/fitLogSelectors";
...
export function useFitLogSelectors(state: State, selectedDate: string) {
  ...
  const splitPartOptions = useMemo(
    () => buildSplitPartOptions(state.exercises, state.workouts, state.trainingDays, state.trainingPlans),
