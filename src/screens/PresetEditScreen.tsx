                  ...
                  return (
                    <div className="preset-row" key={exerciseId}>
                      ...
                      <button
                        ...
                        aria-label="上へ移動"
                        disabled={index === 0}
                        onClick={() => onMove(preset.id, exerciseId, -1)}
                      >
                        <ChevronUp />
                      </button>
                      <button
                      ...
                      <button
                        ...
                        type="button"
                        aria-label="種目を外す"
                        onClick={() => onRemove(preset.id, exerciseId)}
                      >
                        <TrashIcon />
                      </button>
                    </div>
                  ...
                  <div className="preset-add-options">
                    {partExercises.map((exercise) => (
                      <button
                        key={exercise.id}
                        type="button"
                        disabled={preset.exerciseIds.includes(exercise.id)}
                        onClick={() => onAdd(preset.id, exercise.id)}
                      >
                        {exercise.name}
                      </button>
                    ))}
      ...
      {preset && deleteDialogOpen && (
        <div className="dialog-backdrop" role="presentation">
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="preset-edit-delete-title"
          >
            <div id="preset-edit-delete-title" className="confirm-title">
              プリセットを削除しますか？
            </div>
